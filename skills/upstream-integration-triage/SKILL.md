...
Two classes of false evidence poison this triage: sanitised HAR
exports (Chrome 109+ and Safari strip auth headers by default — a
"working call with no auth" may be a lie) and shell-`source`d `.env`
files (values with spaces or `=` get truncated, while systemd and
python-dotenv read the full line). Before trusting a HAR diff or a
shell reproduction, read
[`references/har-and-env-traps.md`](references/har-and-env-traps.md).
...
- `agent-operating-manual` invariant "Read before write" — this skill
  is the vendor-flavoured specialisation of letting the system teach
  you before you form theories.
